...
; NOTE: It is pretty much impossible (in this lifetime) to compute the exact optimal
...
(def enhance-gems-specific
     {:yellow {:bonusAgility 20 :bonusMasteryRating 20}
      :red {:bonusAgility 40}
      :blue {:bonusAgility 20 :bonusHitRating 20}
      :meta {:bonusMasteryRating 54}
      :prismatic {:bonusAgility 40}})
...
(defn consolidate-gear-stats [gear]
  (reduce (fn [r x]
	    (let [slot (key x)
		  item (first (val x))
		  item-stats (filter #(re-find #"^:bonus" (str %)) (keys item))]
	      (reduce #(assoc %1 %2 (conj (get %1 %2 []) (get item %2)))
		      r item-stats)))
	  {}
	  gear))

(defn sum-gem-values [gear gems]
  (reduce (fn [r x]
	    (let [slot (key x)
		  item (first (val x))]
	      (if (contains? item :sockets)
		(merge-with 
		 + 
		 r
...
(defn optimise-reforges [gear gems weights caps]
  (let [gear-stats (consolidate-gear-stats resolved-test-data)
	gem-total (sum-gem-values resolved-test-data enhance-gems-specific)
	base-stats (merge-with + gem-total (reduce #(assoc %1 (key %2) (apply + (val %2)))
						   {}
						   gear-stats))
	need-for-caps (reduce #(assoc %1 (key %2) (- (val %2) (get base-stats (key %2) 0)))
			      {}
			      caps)
	gs-40p (select-keys
		(reduce #(assoc %1 (key %2) (map (fn [i] (int (* 0.4 i))) (val %2)))
			{} gear-stats)
		reforgable-stats)
	rweights (reverse weights)]
    
    (println gear-stats)
    (println gs-40p)
    (println gem-total)
    (println base-stats)
    (println "stats needed for caps:" need-for-caps)
...
(defn run-test []
  (optimise-reforges resolved-test-data 
		     enhance-gems-specific 
		     stat-weights-enhance
		     caps-enhance))
 ; (brute-force-all-options resolved-test-data stat-weights-enhance enhance-gems))
